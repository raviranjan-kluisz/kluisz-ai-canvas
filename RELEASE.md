# Releasing Kluisz Kanvas

Kluisz Kanvas follows a **release-when-ready** cadence, with each cycle typically lasting 4–6 weeks depending on QA and stabilization needs.
...
Create an OSS release candidate (RC) branch containing `kluisz` and any associated PyPI packages (e.g. `klx`).
During this period:
...
* Release timing is coordinated with Kluisz Kanvas's DevRel team.
