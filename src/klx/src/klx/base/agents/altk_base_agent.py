...
from klx.base.agents.utils import data_to_messages, get_chat_output_sender_name
from klx.components.models_and_agents import AgentComponent
...
def normalize_message_content(message: BaseMessage) -> str:
    ...
    Note:
        This addresses the inconsistency in klx.schema.data.Data.to_lc_message() where:
...
class ALTKBaseAgentComponent(AgentComponent):
    ...
    async def run_agent(
        ...
        if hasattr(self, "chat_history") and self.chat_history:
            ...
            # Handle both klx.schema.message.Message and langflow.schema.message.Message types
