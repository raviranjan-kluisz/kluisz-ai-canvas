...
when the same class is re-exported from different modules (e.g., klx.Message vs
...
class CrossModuleMeta(type(BaseModel)):  # type: ignore[misc]
    ...
    def __instancecheck__(cls, instance: Any) -> bool:
        ...
        # 1. Has model_fields attribute on its class (is a Pydantic model)
        # 2. Has the same __class__.__name__
        # 3. Has compatible model fields
        if not hasattr(instance.__class__, "model_fields"):
            return False
        ...
        # Note: Access model_fields from the class, not the instance (Pydantic v2.11+)
        cls_fields = set(cls.model_fields.keys()) if hasattr(cls, "model_fields") else set()
        instance_fields = set(instance.__class__.model_fields.keys())
...
class CrossModuleModel(BaseModel, metaclass=CrossModuleMeta):
    ...
    Example:
        ...
        >>> from klx.schema.message import Message as LfxMessage
