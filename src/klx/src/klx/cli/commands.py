...
from klx.cli.serve_app import FlowMeta, create_multi_serve_app
...
async def serve_command(
    ...
    *,
    ...
    try:
        api_key = get_api_key()
        verbose_print("✓ KLUISZ_API_KEY is configured")
    except ValueError as e:
        typer.echo(f"✗ {e}", err=True)
        typer.echo("Set the KLUISZ_API_KEY environment variable before serving.", err=True)
    ...
    os.environ["KLUISZ_PRETTY_LOGS"] = "false"
    verbose_print(f"Configuring logging with level: {log_level}")
    from klx.log.logger import configure
    ...
    try:
        ...
        if resolved_path.suffix == ".json":
            ...
            from klx.cli.script_loader import load_graph_from_script
        ...
        try:
            ...
            if check_variables:
                from klx.cli.validation import validate_global_variables_for_env
