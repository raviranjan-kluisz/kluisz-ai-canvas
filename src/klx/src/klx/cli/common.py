...
from klx.cli.script_loader import (
    extract_structured_result,
    find_graph_variable,
    load_graph_from_script,
)
from klx.load import load_flow_from_json
...
_KLUISZ_NAMESPACE_UUID = uuid.UUID("3c091057-e799-4e32-8ebc-27bc31e1108c")
...
def get_api_key() -> str:
    ...
    api_key = os.getenv("KLUISZ_API_KEY")
    if not api_key:
        msg = "KLUISZ_API_KEY environment variable is not set"
...
def flow_id_from_path(file_path: Path, root_dir: Path) -> str:
    ...
    return str(uuid.uuid5(_KLUISZ_NAMESPACE_UUID, relative))
