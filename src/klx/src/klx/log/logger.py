...
class SizedLogBuffer:
    ...
    def __init__(
        ...
        The buffer can be overwritten by an env variable KLUISZ_LOG_RETRIEVER_BUFFER_SIZE
    ...
    def max(self) -> int:
        ...
        if self._max == 0:
            env_buffer_size = os.getenv("KLUISZ_LOG_RETRIEVER_BUFFER_SIZE", "0")
...
def configure(
    *,
    ...
    kluisz_log_level = os.getenv("KLUISZ_LOG_LEVEL", "")
    if kluisz_log_level.upper() in VALID_LOG_LEVELS and log_level is None:
        log_level = kluisz_log_level

    log_level_str = kluisz_log_level or "ERROR"
    ...
    if log_file is None:
        env_log_file = os.getenv("KLUISZ_LOG_FILE", "")
        log_file = Path(env_log_file) if env_log_file else None

    if log_env is None:
        log_env = os.getenv("KLUISZ_LOG_ENV", "")

    # Get log format from env if not provided
    if log_format is None:
        log_format = os.getenv("KLUISZ_LOG_FORMAT")
    ...
    # Add callsite information only when KLUISZ_DEV is set
    if DEV:
        processors.append(
    ...
    if log_env.lower() == "container" or log_env.lower() == "container_json":
        ...
        log_stdout_pretty = os.getenv("KLUISZ_PRETTY_LOGS", "true").lower() == "true"
