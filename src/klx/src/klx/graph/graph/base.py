...
from klx.exceptions.component import ComponentBuildError
from klx.graph.edge.base import CycleEdge, Edge
from klx.graph.graph.constants import Finish, lazy_load_vertex_dict
from klx.graph.graph.runnable_vertices_manager import RunnableVerticesManager
from klx.graph.graph.schema import GraphData, GraphDump, StartConfigDict, VertexBuildResult
from klx.graph.graph.state_model import create_state_model_from_graph
from klx.graph.graph.utils import (
...
from klx.graph.schema import InterfaceComponentTypes, RunOutputs
from klx.graph.utils import log_vertex_build
from klx.graph.vertex.base import Vertex, VertexStates
from klx.graph.vertex.schema import NodeData, NodeTypeEnum
from klx.graph.vertex.vertex_types import ComponentVertex, InterfaceVertex, StateVertex
from klx.log.logger import LogConfig, configure, logger
from klx.schema.dotdict import dotdict
from klx.schema.schema import INPUT_FIELD_NAME, InputType, OutputValue
from klx.services.cache.utils import CacheMiss
from klx.services.deps import get_chat_service, get_tracing_service
from klx.utils.async_helpers import run_until_complete
...
if TYPE_CHECKING:
    ...
    from klx.events.event_manager import EventManager
    from klx.graph.edge.schema import EdgeData
    from klx.graph.schema import ResultData
    from klx.schema.schema import InputValueRequest
    from klx.services.chat.schema import GetCache, SetCache
    from klx.services.tracing.service import TracingService
...
class Graph:
    ...
    def __init__(
        ...
        # Kluisz-specific metadata for tracing
        self.kluisz_folder_id: str | None = None  # folder_id (project_id)
        self.kluisz_tenant_id: str | None = None  # tenant_id
    ...
    async def initialize_run(self) -> None:
        ...
        if self.tracing_service:
            ...
            await self.tracing_service.start_tracers(
                ...
                tenant_id=self.kluisz_tenant_id,  # Kluisz tenant context
                kluisz_project_id=self.kluisz_folder_id,  # folder_id (project ID)
                kluisz_flow_id=self.flow_id,  # flow.id
            )
    ...
    async def _log_vertex_build_from_exception(self, vertex_id: str, result: Exception) -> None:
        ...
        if isinstance(result, ComponentBuildError):
            ...
            from klx.utils.exceptions import format_exception_message
