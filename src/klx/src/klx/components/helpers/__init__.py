...
if TYPE_CHECKING:
    from klx.components.helpers.calculator_core import CalculatorComponent
    from klx.components.helpers.create_list import CreateListComponent
    from klx.components.helpers.current_date import CurrentDateComponent
    from klx.components.helpers.id_generator import IDGeneratorComponent
    from klx.components.helpers.memory import MemoryComponent
    from klx.components.helpers.output_parser import OutputParserComponent
    from klx.components.helpers.store_message import MessageStoreComponent
...
# This allows imports like: import klx.components.helpers.current_date
_redirected_submodules = {
    "klx.components.helpers.current_date": "klx.components.utilities.current_date",
    "klx.components.helpers.calculator_core": "klx.components.utilities.calculator_core",
    "klx.components.helpers.id_generator": "klx.components.utilities.id_generator",
    "klx.components.helpers.memory": "klx.components.models_and_agents.memory",
}
...
def __getattr__(attr_name: str) -> Any:
    ...
    # e.g., klx.components.helpers.id_generator -> klx.components.utilities.id_generator
    if attr_name == "id_generator":
        from importlib import import_module

        result = import_module("klx.components.utilities.id_generator")
        globals()[attr_name] = result
        return result
    if attr_name == "calculator_core":
        from importlib import import_module

        result = import_module("klx.components.utilities.calculator_core")
        globals()[attr_name] = result
        return result
    if attr_name == "current_date":
        from importlib import import_module

        result = import_module("klx.components.utilities.current_date")
        globals()[attr_name] = result
        return result
    if attr_name == "memory":
        from importlib import import_module

        result = import_module("klx.components.models_and_agents.memory")
        globals()[attr_name] = result
        return result
    ...
    if attr_name in ("CurrentDateComponent", "CalculatorComponent", "IDGeneratorComponent"):
        from klx.components import utilities
    ...
    if attr_name == "MemoryComponent":
        from klx.components import models_and_agents
    ...
    if attr_name == "CreateListComponent":
        from klx.components import processing
