...
from klx.field_typing.constants import CUSTOM_COMPONENT_SUPPORTED_TYPES, DEFAULT_IMPORT_STRING
from klx.log.logger import logger

_KLUISZ_IS_INSTALLED = False

with contextlib.suppress(ImportError):
    import langflow  # noqa: F401

    _KLUISZ_IS_INSTALLED = True
...
def _create_langflow_execution_context():
    ...
    try:
        from klx.io import HandleInput, Output, TabInput
...
def prepare_global_scope(module):
    ...
    for node in import_froms:
        ...
        if node.module and node.module.startswith("langflow."):
            lfx_module_name = node.module.replace("langflow.", "klx.", 1)
...
def get_default_imports(code_string):
    ...
    langflow_module = importlib.import_module("klx.field_typing")
