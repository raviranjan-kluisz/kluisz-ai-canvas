...
from klx.constants import BASE_COMPONENTS_PATH
from klx.log.logger import logger
from klx.serialization.constants import MAX_ITEMS_LENGTH, MAX_TEXT_LENGTH
from klx.services.settings.constants import AGENTIC_VARIABLES, VARIABLES_TO_GET_FROM_ENVIRONMENT
from klx.utils.util_strings import is_valid_database_url
...
class Settings(BaseSettings):
    # Define the default KLUISZ_DIR
    config_dir: str | None = None
    # Define if kluisz db should be saved in config dir or
    # in the kluisz directory
    save_db_in_config_dir: bool = False
    """Define if kluisz database should be saved in KLUISZ_CONFIG_DIR or in the kluisz directory
    ...
    Controlled by KLUISZ_USE_NOOP_DATABASE env variable."""
    ...
    def set_variables_to_get_from_environment(cls, value):
        ...
        if os.getenv("KLUISZ_AGENTIC_EXPERIENCE", "true").lower() == "true":
    ...
    def set_database_url(cls, value, info):
        ...
        logger.debug("No database_url provided, trying KLUISZ_DATABASE_URL env variable")
        if kluisz_database_url := os.getenv("KLUISZ_DATABASE_URL"):
            value = kluisz_database_url
            logger.debug("Using KLUISZ_DATABASE_URL env variable.")
        else:
            ...
            from klx.utils.version import get_version_info
            from klx.utils.version import is_pre_release as langflow_is_pre_release
            ...
            pre_db_file_name = "kluisz-kanvas-pre.db"
            db_file_name = "kluisz-kanvas.db"
    ...
    def set_components_path(cls, value):
        ...
        If the `KLUISZ_COMPONENTS_PATH` environment variable is set and points to an existing path, it is
        appended to the provided list if not already present. If the input list is empty or missing, it is
        set to an empty list.
        """
        if os.getenv("KLUISZ_COMPONENTS_PATH"):
            logger.debug("Adding KLUISZ_COMPONENTS_PATH to components_path")
            kluisz_component_path = os.getenv("KLUISZ_COMPONENTS_PATH")
            if Path(kluisz_component_path).exists() and kluisz_component_path not in value:
                if isinstance(kluisz_component_path, list):
                    for path in kluisz_component_path:
                        if path not in value:
                            value.append(path)
                    logger.debug(f"Extending {kluisz_component_path} to components_path")
                elif kluisz_component_path not in value:
                    value.append(kluisz_component_path)
                    logger.debug(f"Appending {kluisz_component_path} to components_path")
    ...
    model_config = SettingsConfigDict(validate_assignment=True, extra="ignore", env_prefix="KLUISZ_")
