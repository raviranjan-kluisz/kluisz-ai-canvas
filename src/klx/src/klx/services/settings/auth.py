...
from klx.services.settings.utils import read_secret_from_file, write_secret_to_file
...
class AuthSettings(BaseSettings):
    ...
    """If True, the application will attempt to log in automatically as a super admin."""
    ...
    ENABLE_SUPERADMIN_CLI: bool = Field(
        default=True,
        description="Allow creation of super admins via CLI. Set to False in production for security.",
    )
    """If True, allows creation of super admins via the CLI 'kluisz superadmin' command."""

    NEW_USER_IS_ACTIVE: bool = False
    
    # Super admin credentials (platform-level administrator)
    SUPERADMIN: str = Field(default=DEFAULT_SUPERADMIN, description="Super admin username")
    SUPERADMIN_PASSWORD: SecretStr = Field(default=DEFAULT_SUPERADMIN_PASSWORD, description="Super admin password")
    ...
    def reset_credentials(self) -> None:
        ...
        self.SUPERADMIN_PASSWORD = SecretStr("")

    # If autologin is true, then we need to set the credentials to the default values
    @field_validator("SUPERADMIN", "SUPERADMIN_PASSWORD", mode="before")
    @classmethod
    def validate_superadmin(cls, value, info):
        # When AUTO_LOGIN is enabled, force super admin to use default values.
        if info.data.get("AUTO_LOGIN"):
            logger.debug("Auto login is enabled, forcing super admin to use default values")
            if info.field_name == "SUPERADMIN":
                if value != DEFAULT_SUPERADMIN:
                    logger.debug("Resetting super admin to default value")
                return DEFAULT_SUPERADMIN
            if info.field_name == "SUPERADMIN_PASSWORD":
                if value != DEFAULT_SUPERADMIN_PASSWORD.get_secret_value():
                    logger.debug("Resetting super admin password to default value")
                return DEFAULT_SUPERADMIN_PASSWORD
