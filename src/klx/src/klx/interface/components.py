...
from klx.custom.utils import abuild_custom_components, create_component_template
...
async def _send_telemetry(
    ...
    try:
        ...
        from kluisz.services.telemetry.schema import ComponentIndexPayload
...
async def _load_components_dynamically(
    ...
    try:
        import klx.components as components_pkg
    except ImportError as e:
        await logger.aerror(f"Failed to import klx.components package: {e}", exc_info=True)
        return modules_dict
    ...
    for _, modname, _ in pkgutil.walk_packages(components_pkg.__path__, prefix=components_pkg.__name__ + "."):
        ...
        if target_modules:
            # Extract top-level: "klx.components.mistral.xyz" -> "mistral"
            parts = modname.split(".")
...
def _process_single_module(modname: str) -> tuple[str, dict] | None:
    ...
    try:
        ...
        return None
    # Extract the top-level subpackage name after "klx.components."
    # e.g., "klx.components.Notion.add_content_to_page" -> "Notion"
...
async def load_single_component(component_type: str, component_name: str, components_paths: list[str]):
    ...
    from klx.custom.utils import get_single_component_dict
...
def get_all_components(components_paths, *, as_dict=False):
    """Get all components names combining native and custom components."""
    # Import here to avoid circular imports
    from klx.custom.utils import build_custom_components
