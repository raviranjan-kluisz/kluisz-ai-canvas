...
withEventDeliveryModes(
  ...
  async ({ page }) => {
    ...
    let kluiszCount = await page
      .locator('[data-testid="kluisz-0-option"]')
      .count();
    ...
    while (kluiszCount === 0) {
      ...
      kluiszCount = await page
        .locator('[data-testid="kluisz-0-option"]')
        .count();
    }

    await page.locator('[data-testid="kluisz-0-option"]').nth(0).waitFor({
      timeout: 15000,
      state: "visible",
    });

    await page.getByTestId("kluisz-0-option").nth(0).click();
