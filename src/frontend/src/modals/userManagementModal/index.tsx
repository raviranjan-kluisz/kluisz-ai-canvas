...
export default function UserManagementModal({
  ...
  const [isSuperUser, setIsSuperUser] = useState(data?.is_platform_superadmin ?? false);
  ...
  useEffect(() => {
    if (open) {
      if (!data) {
        ...
        setIsSuperUser(data.is_platform_superadmin);

        handleInput({ target: { name: "username", value: username } });
        handleInput({ target: { name: "is_active", value: isActive } });
        handleInput({ target: { name: "is_platform_superadmin", value: isSuperUser } });
      }
    }
  }, [open]);
        ...
        >
          <div className="grid gap-5">
            ...
            <div className="flex gap-8">
              ...
              {userData?.is_platform_superadmin && (
                <Form.Field name="is_platform_superadmin">
                  <div>
                    <Form.Label className="data-[invalid]:label-invalid mr-3">
                      Superuser
                    </Form.Label>
                    <Form.Control asChild>
                      <Checkbox
                        checked={isSuperUser}
                        value={isSuperUser}
                        id="is_platform_superadmin"
                        className="relative top-0.5"
                        onCheckedChange={(value) => {
                          handleInput({
                            target: { name: "is_platform_superadmin", value },
                          });
