...
export const SidebarHeaderComponent = memo(function SidebarHeaderComponent({
  ...
  return (
    ...
    <SidebarHeader className="relative flex w-full flex-col gap-2 group-data-[collapsible=icon]:hidden border-b">
      {/* Toggle Sidebar Button - Positioned absolutely at top right */}
      <ShadTooltip
        content={open ? "Hide sidebar" : "Show sidebar"}
        styleClasses="z-50"
        side="right"
      >
        <Button
          ...
          variant="ghost"
          size="icon"
          className="absolute -right-[20px] rounded-sm top-1 z-10 h-6 w-6 border border-border bg-background text-muted-foreground shadow-sm hover:bg-accent"
