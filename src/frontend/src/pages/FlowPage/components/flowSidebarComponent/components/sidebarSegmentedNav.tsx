...
import { SIDEBAR_SEGMENT_FEATURES } from "@/constants/feature-maps";
...
const SidebarSegmentedNav = () => {
  ...
  const visibleNavItems = useMemo(() => {
    return NAV_ITEMS.filter((item) => {
      const requiredFeatures = SIDEBAR_SEGMENT_FEATURES[item.id] || [];
      // If no features required, always show
      if (requiredFeatures.length === 0) return true;
      // If any required feature is enabled, show the item (OR logic)
      return requiredFeatures.some((feature) => isFeatureEnabled(feature));
    });
  }, [isFeatureEnabled]);
