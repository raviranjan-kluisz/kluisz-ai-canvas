...
import { useFilteredComponentsByCategory } from "@/hooks/features";
import { useSidebarSegmentFeatures } from "@/hooks/features";
...
export function FlowSidebarComponent({ isLoading }: FlowSidebarComponentProps) {
  ...
  // Get sidebar segment visibility
  const { isSegmentEnabled } = useSidebarSegmentFeatures();

  // Step 1: Filter out knowledge components when ENABLE_KNOWLEDGE_BASES is OFF
  const knowledgeFilteredData = useMemo(() => {
  ...
  // Step 2: Apply feature-based filtering using the centralized hook
  // This automatically filters categories and components based on feature-maps.ts
  const data = useFilteredComponentsByCategory(knowledgeFilteredData as any);
