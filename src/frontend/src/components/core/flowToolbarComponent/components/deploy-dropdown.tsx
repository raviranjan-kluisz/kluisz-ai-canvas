...
import { useFlowToolbarFeatures, useIntegrationFeatures } from "@/hooks/features";
...
export default function PublishDropdown({
  ...
  // Use centralized feature hooks instead of FeatureGate components
  const { isActionEnabled } = useFlowToolbarFeatures();
  const { isIntegrationEnabled } = useIntegrationFeatures();
  
  const canExport = isActionEnabled("export");
  const canShare = isActionEnabled("share");
  const canUseMcp = isIntegrationEnabled("mcp");
  const canAccessApi = isActionEnabled("api-access");
  const canEmbedWidget = isActionEnabled("embed-widget");
