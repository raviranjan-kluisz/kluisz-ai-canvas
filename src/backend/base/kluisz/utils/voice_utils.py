...
# Lazy import for scipy - only loaded when voice features are used
_resample = None


def _get_resample():
    """Lazy import of scipy.signal.resample to avoid import errors when scipy is not installed."""
    global _resample
    if _resample is None:
        try:
            ...
            msg = (
                "scipy is required for voice features. "
                "Install it with: pip install scipy>=1.15.1"
            )
