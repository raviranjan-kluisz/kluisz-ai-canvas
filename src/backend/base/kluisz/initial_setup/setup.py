...
from klx.base.constants import (
...
from klx.template.field.prompt import DEFAULT_PROMPT_INTUT_TYPES
from klx.utils.util import escape_json_dump
from sqlalchemy.exc import NoResultFound
from sqlalchemy.orm import selectinload
from sqlmodel import col, select
from sqlmodel.ext.asyncio.session import AsyncSession

from kluisz.initial_setup.constants import (
...
from kluisz.services.database.models.flow.model import Flow, FlowCreate
from kluisz.services.database.models.folder.constants import (
    DEFAULT_FOLDER_DESCRIPTION,
    DEFAULT_FOLDER_NAME,
    LEGACY_FOLDER_NAMES,
)
from kluisz.services.database.models.folder.model import Folder, FolderCreate, FolderRead
from kluisz.services.deps import get_settings_service, get_storage_service, get_variable_service, session_scope
...
async def create_or_update_agentic_flows(session: AsyncSession, user_id: UUID) -> None:
    ...
    """
    from klx.services.deps import get_settings_service
...
async def load_flows_from_directory() -> None:
    ...
    All flows are uploaded into the default folder for the super admin.
    """
    settings_service = get_settings_service()
    ...
    async with session_scope() as session:
        # Find super admin by role instead of username to avoid issues with credential reset
        from kluisz.services.database.models.user.model import User

        stmt = select(User).where(User.is_platform_superadmin == True)  # noqa: E712
        result = await session.exec(stmt)
        user = result.first()
        if user is None:
            msg = "No super admin found in the database"
...
async def initialize_auto_login_default_superadmin() -> None:
    settings_service = get_settings_service()
    ...
    if not username or not password:
        msg = "SUPERADMIN and SUPERADMIN_PASSWORD must be set in the settings if AUTO_LOGIN is true."
        raise ValueError(msg)
    ...
    async with session_scope() as async_session:
        ...
        # Seed feature registry if tables exist
        await seed_feature_registry_if_needed(async_session)
    await logger.adebug("Super user initialized")


async def seed_feature_registry_if_needed(session) -> None:
    """Seed the feature registry with default features if the table exists and is empty."""
    try:
        from kluisz.initial_setup.seed_features import seed_feature_registry, seed_default_model_registry
