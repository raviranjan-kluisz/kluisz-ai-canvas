...
from kluisz.schema.content_block import ContentBlock
from kluisz.schema.properties import Properties
from kluisz.schema.validators import str_to_timestamp_validator

if TYPE_CHECKING:
    from kluisz.schema.message import Message
...
class MessageBase(SQLModel):
    ...
    def from_message(cls, message: "Message", flow_id: str | UUID | None = None):
        # Extract all attributes into local variables first to prevent SQLAlchemy
        # from inspecting the message object (which could be an ErrorMessage)
        message_text = getattr(message, "text", None)
        message_sender = getattr(message, "sender", None)
        message_sender_name = getattr(message, "sender_name", None)
        message_files = getattr(message, "files", None)
        ...
        message_flow_id = getattr(message, "flow_id", None)
        message_properties = getattr(message, "properties", None)
        message_category = getattr(message, "category", "message")
        message_content_blocks = getattr(message, "content_blocks", None)

        # first check if the record has all the required fields
        if message_text is None or not message_sender or not message_sender_name:
            msg = "The message does not have the required fields (text, sender, sender_name)."
            raise ValueError(msg)
        
        files_list = message_files or []
        if files_list:
            image_paths = []
            ...
            if image_paths:
                files_list = image_paths

        if isinstance(message_timestamp, str):
            # Convert timestamp string in format "YYYY-MM-DD HH:MM:SS UTC" to datetime
            try:
                timestamp = datetime.strptime(message_timestamp, "%Y-%m-%d %H:%M:%S %Z").replace(tzinfo=timezone.utc)
            except ValueError:
                # Fallback for ISO format if the above fails
                timestamp = datetime.fromisoformat(message_timestamp).replace(tzinfo=timezone.utc)
        else:
            timestamp = message_timestamp
        
        if not flow_id and message_flow_id:
            flow_id = message_flow_id
        
        # If the text is not a string, it means it could be
        # async iterator so we simply add it as an empty string
        message_text = "" if not isinstance(message_text, str) else message_text

        properties = (
            message_properties.model_dump_json()
            if hasattr(message_properties, "model_dump_json")
            else message_properties
        )
        content_blocks = []
        for content_block in (message_content_blocks or []):
