"""Feature Control Service - Manages user feature flags based on license tiers."""
...
class FeatureControlService(Service):
    ...
    Global Registry -> License Tier -> User (via tier)

    Features:
    - Fast resolution with caching
    - Audit logging for all changes
    - Dependency checking
    - Simple model: users inherit features from their assigned tier
    """
        ...
        async with session_scope() as session:
            ...
            if user.license_tier_id:
                ...
                if tier:
                    tier_id = str(tier.id)
                    tier_name = tier.name
                    features = await self._apply_tier_features(session, features, tier.id)
    ...
    async def get_enabled_models(
        ...
        async with session_scope() as session:
            # Get all models
            stmt = select(ModelRegistry).where(ModelRegistry.is_active == True)  # noqa: E712
    ...
    async def get_enabled_components(
        ...
        async with session_scope() as session:
            stmt = select(ComponentRegistry).where(ComponentRegistry.is_active == True)  # noqa: E712
            result = await session.exec(stmt)
            ...
            for comp in components:
                ...
                if comp.required_features:
                    all_required = all(
                        features["features"].get(f, {}).get("enabled", False) for f in comp.required_features
                    )
                    if not all_required:
                        continue
        ...
        *,
        ...
        async with session_scope() as session:
            for feature_key, value in features.items():
                ...
                registry_stmt = select(FeatureRegistry).where(FeatureRegistry.feature_key == feature_key)
    ...
    async def get_tier_features(
        ...
        Returns:
            Feature key -> value mapping
        """
        from kluisz.services.database.models.feature.model import LicenseTierFeatures

        async with session_scope() as session:
            stmt = select(LicenseTierFeatures).where(LicenseTierFeatures.license_tier_id == str_to_uuid(tier_id))
            result = await session.exec(stmt)
            return {f.feature_key: f.feature_value for f in result.all()}
    ...
    async def _get_global_defaults(
        ...
        stmt = select(FeatureRegistry).where(FeatureRegistry.is_active == True)  # noqa: E712
        result = await session.exec(stmt)
        features = {}
        ...
        for reg in result.all():
            features[reg.feature_key] = {
                "enabled": (
                    reg.default_value.get("enabled", False)
                    if isinstance(reg.default_value, dict)
                    else bool(reg.default_value)
                ),
    ...
    async def _apply_tier_features(
        ...
        stmt = select(LicenseTierFeatures).where(LicenseTierFeatures.license_tier_id == tier_id)
        result = await session.exec(stmt)
