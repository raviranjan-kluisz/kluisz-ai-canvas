...
if TYPE_CHECKING:
    ...
    from klx.custom.custom_component.component import Component
...
def _get_langsmith_tracer():
    from kluisz.services.tracing.langsmith import LangSmithTracer

    return LangSmithTracer


def _get_langwatch_tracer():
    from kluisz.services.tracing.langwatch import LangWatchTracer

    return LangWatchTracer


def _get_langfuse_tracer():
    from kluisz.services.tracing.langfuse import LangFuseTracer

    return LangFuseTracer


def _get_arize_phoenix_tracer():
    from kluisz.services.tracing.arize_phoenix import ArizePhoenixTracer

    return ArizePhoenixTracer


def _get_opik_tracer():
    from kluisz.services.tracing.opik import OpikTracer

    return OpikTracer


def _get_traceloop_tracer():
    from kluisz.services.tracing.traceloop import TraceloopTracer
...
class TraceContext:
    """Context for a trace, including Kluisz-specific metadata."""
    ...
    def __init__(
        ...
        self.kluisz_project_id: str | None = kluisz_project_id  # folder_id
        self.kluisz_flow_id: str | None = kluisz_flow_id  # flow.id
        ...
        # Metering callback for local billing (real-time credit deduction)
        self.metering_callback: Any | None = None
...
class TracingService(Service):
    ...
    def _initialize_langfuse_tracer(self, trace_context: TraceContext) -> None:
        ...
        trace_context.tracers["langfuse"] = langfuse_tracer(
            ...
            tenant_id=trace_context.tenant_id,  # Kluisz tenant context
            kluisz_project_id=trace_context.kluisz_project_id,  # folder_id
            kluisz_flow_id=trace_context.kluisz_flow_id,  # flow.id
        )
    ...
    async def start_tracers(
        ...
        - create a trace context with Kluisz metadata
        ...
        Args:
            ...
            tenant_id: Kluisz tenant ID (auto-fetched from user if not provided)
            kluisz_project_id: folder_id from Flow model
            kluisz_flow_id: flow.id from Flow model
        """
        if self.deactivated:
            return
        try:
            project_name = project_name or os.getenv("LANGCHAIN_PROJECT", "Kluisz")
            
            # Auto-fetch tenant_id from user if not provided
            if user_id and not tenant_id:
                try:
                    ...
                    async with session_scope() as session:
                        user = await get_user_by_id(session, UUIDType(user_id))
                        if user and user.tenant_id:
                            tenant_id = str(user.tenant_id)
                except Exception as e:  # noqa: BLE001
                    logger.debug(f"Could not fetch tenant_id for user {user_id}: {e}")
    ...
    async def end_tracers(self, outputs: dict, error: Exception | None = None) -> None:
        ...
        - finalize metering and deduct credits
        """
        if self.deactivated:
            return
        ...
        # Finalize metering and deduct credits (local billing)
        # This happens AFTER the trace ends, with all accumulated usage
        if trace_context.metering_callback is not None:
            print(f"\n[METERING] Calling finalize_and_deduct...")
            try:
                result = await trace_context.metering_callback.finalize_and_deduct()
                print(f"[METERING] Result: {result}")
                if result.get("credits_deducted", 0) > 0:
                    print(
                        f"[METERING] ✅ Deducted {result.get('credits_deducted')} credits "
                        f"(${result.get('cost_usd', 0):.6f}, {result.get('tokens', 0)} tokens)"
                    )
                elif result.get("error"):
                    print(f"[METERING] ❌ Error: {result.get('error')}")
                elif result.get("reason"):
                    print(f"[METERING] ⚠️ No deduction: {result.get('reason')}")
            except Exception as e:
                print(f"[METERING] ❌ Error finalizing metering: {e}")
                import traceback
                traceback.print_exc()
        else:
            print(f"\n[METERING] ⚠️ No metering callback found in trace context (user_id may be None)")
    ...
    def get_langchain_callbacks(self) -> list[BaseCallbackHandler]:
        ...
        print(f"\n[METERING] get_langchain_callbacks called, user_id={trace_context.user_id[:8] if trace_context.user_id else 'None'}...")
        
        # Add callbacks from all tracers (Langfuse, LangSmith, etc.)
        active_tracers = []
        skipped_tracers = []
        for tracer_name, tracer in trace_context.tracers.items():
            if not tracer.ready:  # type: ignore[truthy-function]
                skipped_tracers.append(tracer_name)
                continue
            langchain_callback = tracer.get_langchain_callback()
            if langchain_callback:
                callbacks.append(langchain_callback)
                active_tracers.append(tracer_name)
        
        if active_tracers:
            print(f"[TRACING] Active tracers: {', '.join(active_tracers)}")
        if skipped_tracers:
            print(f"[TRACING] Skipped tracers (not ready): {', '.join(skipped_tracers)}")
        
        # Add Kluisz metering callback for local billing
        # This captures LLM usage in real-time for credit deduction
        if trace_context.user_id and trace_context.metering_callback is None:
            try:
                from kluisz.services.tracing.metering_callback import KluiszMeteringCallback
                metering_callback = KluiszMeteringCallback(
                    user_id=trace_context.user_id,
                    tenant_id=trace_context.tenant_id,
                    flow_id=trace_context.kluisz_flow_id,
                    trace_id=str(trace_context.run_id) if trace_context.run_id else None,
                )
                callbacks.append(metering_callback)
                # Store reference for finalization at end of trace
                trace_context.metering_callback = metering_callback
                print(f"\n[METERING] Created metering callback for user={trace_context.user_id[:8]}...")
            except Exception as e:
                logger.warning(f"Could not initialize metering callback: {e}")
        elif trace_context.metering_callback is not None:
            # Reuse existing metering callback
            callbacks.append(trace_context.metering_callback)
            print(f"[METERING] Reusing existing metering callback")
        else:
            print(f"[METERING] ⚠️ Metering callback NOT created: user_id={trace_context.user_id}")
        
        print(f"[METERING] Total callbacks: {len(callbacks)}")
        return callbacks
