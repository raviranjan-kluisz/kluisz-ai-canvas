...
from klx.services.deps import injectable_session_scope, session_scope
from klx.services.settings.service import SettingsService
from sqlalchemy.exc import IntegrityError
from sqlmodel.ext.asyncio.session import AsyncSession
from starlette.websockets import WebSocket

from kluisz.helpers.user import get_user_by_flow_id_or_endpoint_name
from kluisz.services.database.models.api_key.crud import check_key
from kluisz.services.database.models.user.crud import get_user_by_id, get_user_by_username, update_user_last_login_at
...
AUTO_LOGIN_WARNING = "In v2.0, KLUISZ_SKIP_AUTH_AUTO_LOGIN will be removed. Please update your authentication method."
AUTO_LOGIN_ERROR = (
    "Since v1.5, KLUISZ_AUTO_LOGIN requires a valid API key. "
    "Set KLUISZ_SKIP_AUTH_AUTO_LOGIN=true to skip this check. "
...
async def api_key_security(
    ...
    async with session_scope() as db:
        if settings_service.auth_settings.AUTO_LOGIN:
            # Get the first user
            if not settings_service.auth_settings.SUPERADMIN:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Missing first super admin credentials",
                )
            if not query_param and not header_param:
                if settings_service.auth_settings.skip_auth_auto_login:
                    result = await get_user_by_username(db, settings_service.auth_settings.SUPERADMIN)
...
async def ws_api_key_security(
    ...
    async with session_scope() as db:
        if settings.auth_settings.AUTO_LOGIN:
            if not settings.auth_settings.SUPERADMIN:
                # internal server misconfiguration
                raise WebSocketException(
                    code=status.WS_1011_INTERNAL_ERROR,
                    reason="Missing first super admin credentials",
                )
            if not api_key:
                if settings.auth_settings.skip_auth_auto_login:
                    result = await get_user_by_username(db, settings.auth_settings.SUPERADMIN)
                    logger.warning(AUTO_LOGIN_WARNING)
                else:
                    raise WebSocketException(
...
async def get_current_active_superuser(current_user: Annotated[User, Depends(get_current_user)]) -> User:
    """Get current user if they are a super admin (platform level)."""
    if not current_user.is_active:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Inactive user")
...
# Alias for clarity
get_current_super_admin = get_current_active_superuser


async def get_current_tenant_admin(
    current_user: Annotated[User, Depends(get_current_user)],
) -> User:
    """Get current user if they are a tenant admin or super admin.
...
async def check_tenant_access(user: User, tenant_id: UUID) -> None:
    """Check if user has access to a specific tenant.
    
    Super admins have access to all tenants.
    Other users only have access to their own tenant.
...
async def check_tenant_admin_access(user: User, tenant_id: UUID) -> None:
    """Check if user has admin access to a specific tenant.
    
    Super admins have admin access to all tenants.
...
async def create_super_user(
    ...
    if not super_user:
        ...
        try:
            ...
            logger.debug("Error creating super admin.", exc_info=True)
...
async def create_user_longterm_token(db: AsyncSession) -> tuple[UUID, dict]:
    ...
    # Prefer configured username; fall back to default or any existing super admin
    # NOTE: This user name cannot be a dynamic current user name since it is only used when autologin is True
    username = settings_service.auth_settings.SUPERADMIN
    super_user = await get_user_by_username(db, username)
    if not super_user:
        from kluisz.services.database.models.user.crud import get_all_superadmins

        superadmins = await get_all_superadmins(db)
        super_user = superadmins[0] if superadmins else None
...
async def get_current_user_mcp(
    ...
    - If no API key is provided and AUTO_LOGIN is enabled, it falls back to
      username lookup using the configured super admin credentials
