...
# Increase recursion limit for PostgreSQL/SQLAlchemy compatibility
# This helps prevent "maximum recursion depth exceeded" errors
# when using async PostgreSQL drivers with complex model relationships
sys.setrecursionlimit(5000)

# Also set greenlet stack limit if greenlet is available (used by psycopg async driver)
try:
    import greenlet
    # greenlet has no public API to set stack limit, but we can try
except ImportError:
    pass
...
from kluisz.cli.progress import create_kluisz_progress
from kluisz.initial_setup.setup import get_or_create_default_folder
from kluisz.main import setup_app
from kluisz.services.auth.utils import check_key, get_current_user_by_jwt
from kluisz.services.deps import get_db_service, get_settings_service, is_settings_service_initialized, session_scope
from kluisz.services.utils import initialize_services
from kluisz.utils.version import fetch_latest_version, get_version_info
from kluisz.utils.version import is_pre_release as kluisz_is_pre_release
...
# Add KLX commands as a sub-app
try:
    from klx.cli.commands import serve_command
    from klx.cli.run import run as klx_run

    klx_app = typer.Typer(name="klx", help="Kluisz Executor commands")
    klx_app.command(name="serve", help="Serve a flow as an API", no_args_is_help=True)(serve_command)
    klx_app.command(name="run", help="Run a flow directly", no_args_is_help=True)(klx_run)

    app.add_typer(klx_app, name="klx")
except ImportError:
    # KLX not available, skip adding the sub-app
    pass
...
def wait_for_server_ready(host, port, protocol, timeout: int = 300) -> None:
    ...
    logger.info(f"Waiting for server at {protocol}://{health_check_host}:{port}/health (timeout: {timeout}s)")
    ...
    while status_code != httpx.codes.OK:
        ...
        if elapsed > timeout:
            error_msg = (
                ...
                f"Made {attempt} attempts. Last status code: {status_code}"
            )
        ...
        try:
            ...
            elif attempt % 10 == 0:  # Log every 10 attempts
                logger.debug(f"Server not ready yet (attempt {attempt}, elapsed {elapsed:.1f}s, status: {status_code})")
        except HTTPError as e:
            if attempt % 10 == 0:
                logger.debug(f"HTTP error waiting for server (attempt {attempt}, elapsed {elapsed:.1f}s): {e}")
            time.sleep(1)
        except Exception as e:  # noqa: BLE001
            if attempt % 10 == 0:
                logger.debug(f"Error waiting for server (attempt {attempt}, elapsed {elapsed:.1f}s): {e}", exc_info=True)
            time.sleep(1)
...
def run(
    *,
    ...
    log_level = (log_level or os.environ.get("KLUISZ_LOG_LEVEL") or "info").lower()
    os.environ["KLUISZ_LOG_LEVEL"] = log_level
    ...
    progress = create_kluisz_progress(verbose=verbose)

    # Step 0: Initializing Kluisz
    ...
    if platform.system() == "Windows":
        ...
        # Check if we should use simple uvicorn (for debugging/compatibility)
        use_simple_server = os.environ.get("KLUISZ_USE_SIMPLE_SERVER", "").lower() in ("true", "1", "yes")
        
        if use_simple_server:
            # Use simple uvicorn server (single worker, no Gunicorn)
            # This avoids potential issues with multiprocess/fork
            with progress.step(6):
                import uvicorn

                logger.info(f"Starting uvicorn server on {host}:{port} (simple mode)")
                progress.print_summary()
                print_banner(str(host), int(port or 7860), protocol)
            ...
            uvicorn.run(
                ...
                reload=False,
                workers=1,  # Single worker in simple mode
                loop="asyncio",
            )
        else:
            with progress.step(6):
                # Use Gunicorn with KluiszUvicornWorker for non-Windows systems
                from kluisz.server import KluiszApplication
                ...
                logger.info(f"Starting Gunicorn with {get_number_of_workers(workers)} workers on {host}:{port}")
                server = KluiszApplication(app, options)

                # Start the webapp process
                logger.debug("Creating webapp process...")
                process_manager.webapp_process = Process(target=server.run)
                logger.debug("Starting webapp process...")
                process_manager.webapp_process.start()
                logger.debug(f"Webapp process started with PID: {process_manager.webapp_process.pid}")
                
                # Check if process is alive
                time.sleep(0.5)
                if not process_manager.webapp_process.is_alive():
                    logger.error("Webapp process died immediately after starting!")
                    logger.error(f"Process exitcode: {process_manager.webapp_process.exitcode}")
                    raise RuntimeError("Gunicorn process failed to start")

                logger.info("Waiting for server to become ready...")
                try:
                    wait_for_server_ready(host, port, protocol, timeout=300)
                except TimeoutError as e:
                    logger.error(f"Server startup timeout: {e}")
                    # Check if process is still alive
                    if process_manager.webapp_process.is_alive():
                        logger.error("Process is alive but not responding to health checks")
                    else:
                        logger.error(f"Process died. Exitcode: {process_manager.webapp_process.exitcode}")
                    raise
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "kluisz")
        'A new version of kluisz is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=kluisz_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if kluisz_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use kluisz.utils.version to get the version info
    version_info = get_version_info()
    kluisz_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= kluisz_is_pre_release(kluisz_version)  # Update pre-release status

    notice = build_version_notice(kluisz_version, package_name)
    ...
    title = f"[bold]Welcome to Kluisz Kanvas[/bold]\n"
    ...
    info_text = (
        f"{github_icon} GitHub: Star for updates {arrow} https://github.com/kluisz/kluisz-ai-canvas\n"
        f"{discord_icon} Discord: Join for support {arrow} https://discord.gg/kluisz"
    )
    telemetry_text = (
        (
            "We collect anonymous usage data to improve Kluisz Kanvas.\n"
            "To opt out, set: [bold]DO_NOT_TRACK=true[/bold] in your environment."
        )
        if os.getenv("DO_NOT_TRACK", os.getenv("KLUISZ_DO_NOT_TRACK", "False")).lower() != "true"
        else (
            "We are [bold]not[/bold] collecting anonymous usage data to improve Kluisz Kanvas.\n"
            "To contribute, set: [bold]DO_NOT_TRACK=false[/bold] in your environment."
        )
    )
    access_host = get_best_access_host(host, port)
    access_link = f"[bold]{status_icon} Open Kluisz Kanvas {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
    ...
    try:
        ...
        fallback_message = (
            f"Welcome to Kluisz Kanvas\n\n"
            "* GitHub: https://github.com/kluisz/kluisz-ai-canvas\n"
            "# Discord: https://discord.gg/kluisz\n\n"
            f"{telemetry_text}\n\n"
            f"[OK] Open Kluisz Kanvas -> {protocol}://{access_host}:{port}"
        )
        try:
            ...
            logger.info("Welcome to Kluisz Kanvas")
            logger.info("GitHub: https://github.com/kluisz/kluisz-ai-canvas")
            logger.info("Discord: https://discord.gg/kluisz")
            logger.info(f"Open Kluisz Kanvas: {protocol}://{access_host}:{port}")


@app.command()
def superadmin(
    username: str = typer.Option(
        None, help="Username for the super admin. Defaults to 'admin' when AUTO_LOGIN is enabled."
    ),
    password: str = typer.Option(
        None, help="Password for the super admin. Defaults to 'admin' when AUTO_LOGIN is enabled."
    ),
    log_level: str = typer.Option("error", help="Logging level.", envvar="KLUISZ_LOG_LEVEL"),
    auth_token: str = typer.Option(
        None, help="Authentication token of existing super admin.", envvar="KLUISZ_SUPERADMIN_TOKEN"
    ),
) -> None:
    ...
    asyncio.run(_create_superadmin(username, password, auth_token))


async def _create_superadmin(username: str, password: str, auth_token: str | None):
    """Create a super admin."""
    await initialize_services()

    settings_service = get_settings_service()
    # Check if super admin creation via CLI is enabled
    if not settings_service.auth_settings.ENABLE_SUPERADMIN_CLI:
        typer.echo("Error: Superuser creation via CLI is disabled.")
        typer.echo("Set KLUISZ_ENABLE_SUPERADMIN_CLI=true to enable this feature.")
        raise typer.Exit(1)
    ...
    from kluisz.services.database.models.user.crud import get_all_superadmins

    existing_superadmins = []
    async with session_scope() as session:
        # Note that the default super admin is created by the initialize_services() function,
        # but leaving this check here in case we change that behavior
        existing_superadmins = await get_all_superadmins(session)
    is_first_setup = len(existing_superadmins) == 0

    # If AUTO_LOGIN is true, only allow default super admin creation
    if settings_service.auth_settings.AUTO_LOGIN:
        if not is_first_setup:
            typer.echo("Error: Cannot create additional super admins when AUTO_LOGIN is enabled.")
            typer.echo("AUTO_LOGIN mode is for development with only the default super admin.")
            typer.echo("To create additional super admins:")
            typer.echo("1. Set KLUISZ_AUTO_LOGIN=false")
            typer.echo("2. Run this command again with --auth-token")
            raise typer.Exit(1)

        typer.echo(f"AUTO_LOGIN enabled. Creating default super admin '{username}'...")
        # Do not echo the default password to avoid exposing it in logs.
    # AUTO_LOGIN is false - production mode
    elif is_first_setup:
        typer.echo("No super admins found. Creating first super admin...")
    else:
        # Authentication is required in production mode
        if not auth_token:
            typer.echo("Error: Creating a super admin requires authentication.")
            typer.echo("Please provide --auth-token with a valid super admin API key or JWT token.")
            typer.echo("To get a token, use: `uv run kluisz api_key`")
            raise typer.Exit(1)
        ...
        try:
            ...
            async with session_scope() as session:
                ...
                try:
                    ...
                    if api_key_result and hasattr(api_key_result, "is_platform_superadmin"):
                        user = api_key_result
                auth_user = user

            if not auth_user or not auth_user.is_platform_superadmin:
                typer.echo(
                    "Error: Invalid token or insufficient privileges. Only super admins can create other super admins."
                )
                raise typer.Exit(1)
        except typer.Exit:
    ...
    # Auth complete, create the super admin
    async with session_scope() as session:
        from kluisz.services.auth.utils import create_super_user

        if await create_super_user(db=session, username=username, password=password):
            # Verify that the super admin was created
            from kluisz.services.database.models.user.model import User

            stmt = select(User).where(User.username == username)
            created_user: User = (await session.exec(stmt)).first()
            if created_user is None or not created_user.is_platform_superadmin:
            ...
            # Log the super admin creation for audit purposes
            logger.warning(
                f"SECURITY AUDIT: New super admin '{username}' created via CLI command"
                + (" by authenticated user" if auth_token else " (first-time setup)")
            )
            typer.echo("Superuser created successfully.")

        else:
            logger.error(f"SECURITY AUDIT: Failed attempt to create super admin '{username}' via CLI")
            typer.echo("Superuser creation failed.")


# command to copy the kluisz database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'kluisz.db' and 'kluisz-pre.db' files from the cache directory to the current
    directory.
    ...
    cache_dir = Path(user_cache_dir("kluisz"))
    db_path = cache_dir / "kluisz.db"
    pre_db_path = cache_dir / "kluisz-pre.db"
...
def api_key(
    ...
    """Creates an API key for the default super admin if AUTO_LOGIN is enabled.
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            stmt = select(User).where(User.username == DEFAULT_SUPERADMIN)
            superadmin = (await session.exec(stmt)).first()
            if not superadmin:
                typer.echo(
                    "Default super admin not found. This command requires a super admin and AUTO_LOGIN to be enabled."
                )
                return None
            from kluisz.services.database.models.api_key.crud import create_api_key, delete_api_key
            from kluisz.services.database.models.api_key.model import ApiKey, ApiKeyCreate

            stmt = select(ApiKey).where(ApiKey.user_id == superadmin.id)
            api_key = (await session.exec(stmt)).first()
            if api_key:
                await delete_api_key(session, api_key.id)

            api_key_create = ApiKeyCreate(name="CLI")
            return await create_api_key(session, api_key_create, user_id=superadmin.id)
