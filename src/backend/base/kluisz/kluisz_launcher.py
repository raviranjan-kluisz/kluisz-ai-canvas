...
def main():
    """Launches kluisz with appropriate environment setup.
    ...
    if platform.system() == "Darwin":  # macOS
        ...
        from kluisz.__main__ import main as kluisz_main

        kluisz_main()


def _launch_with_exec():
    """Launch kluisz by replacing current process with properly configured environment.
    ...
    try:
        os.execv(sys.executable, [sys.executable, "-m", "kluisz.__main__", *sys.argv[1:]])  # noqa: S606
    except OSError as e:
        # If exec fails, we need to exit since the process replacement failed
        typer.echo(f"Failed to exec kluisz: {e}", file=sys.stderr)
        sys.exit(1)
