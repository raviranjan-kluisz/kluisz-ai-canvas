...
def upgrade() -> None:
    ...
    try:
        if db_dialect == "sqlite":
            ...
            # Allow tenant_id column if it exists (from multi-tenant migration)
            expected_base = ['id', 'user_id', 'name', 'path', 'size', 'provider', 'created_at', 'updated_at']
            expected_with_tenant = expected_base + ['tenant_id']
            if set(cols) != set(expected_base) and set(cols) != set(expected_with_tenant):
            ...
            if has_tenant_id:
                ...
                """)
                # Copy data including tenant_id
                op.execute("""
                    INSERT INTO file_new (id, user_id, tenant_id, name, path, size, provider, created_at, updated_at)
                    SELECT id, user_id, tenant_id, name, path, size, provider, created_at, updated_at
                    FROM file
                """)
            else:
                op.execute("""
