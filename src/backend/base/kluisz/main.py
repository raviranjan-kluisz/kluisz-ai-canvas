...
from klx.interface.utils import setup_llm_caching
...
from kluisz.api import health_check_router, log_router, router
from kluisz.api.v1.mcp_projects import init_mcp_servers
from kluisz.initial_setup.setup import (
    copy_profile_pictures,
    create_or_update_starter_projects,
    initialize_auto_login_default_superadmin,
    load_bundles_from_urls,
    load_flows_from_directory,
    sync_flows_from_fs,
)
from kluisz.middleware import ContentSizeLimitMiddleware
from kluisz.services.deps import (
    get_queue_service,
    get_service,
...
from kluisz.services.utils import initialize_services, initialize_settings_service, teardown_services

if TYPE_CHECKING:
    from tempfile import TemporaryDirectory

    from klx.services.mcp_composer.service import MCPComposerService
...
# Suppress SQLAlchemy 2.0 relationship type annotation warnings for SQLModel
# This must be done BEFORE importing any models that use relationships
# SQLModel handles relationships differently than pure SQLAlchemy 2.0
warnings.filterwarnings(
    "ignore",
    message=".*seems to be using a generic class as the argument to relationship.*",
...
def warn_about_future_cors_changes(settings):
    ...
    if using_defaults:
        logger.warning(
            ...
            "Consider setting KLUISZ_CORS_ORIGINS for production deployments. "
    ...
    if settings.cors_origins == "*" and settings.cors_allow_credentials:
        logger.warning(
            ...
            "Specify exact origins in KLUISZ_CORS_ORIGINS to use credentials securely."
        )
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        from klx.interface.components import get_and_cache_all_types_dict

        configure()

        # Startup message
        if version:
            await logger.adebug(f"Starting Kluisz Kanvas v{version}...")
        else:
            await logger.adebug("Starting Kluisz Kanvas...")
        ...
        try:
            ...
            if get_settings_service().auth_settings.AUTO_LOGIN:
                current_time = asyncio.get_event_loop().time()
                await logger.adebug("Initializing default super admin")
                await initialize_auto_login_default_superadmin()
                await logger.adebug(
                    f"Default super admin initialized in {asyncio.get_event_loop().time() - current_time:.2f}s"
                )
            else:
                await logger.adebug("AUTO_LOGIN is disabled, skipping super user initialization")
            ...
            lock_file = Path(tempfile.gettempdir()) / "kluisz_starter_projects.lock"
            ...
            if get_settings_service().settings.agentic_experience:
                from kluisz.api.utils.mcp.agentic_mcp import initialize_agentic_global_variables
            ...
            if get_settings_service().settings.agentic_experience:
                from kluisz.api.utils.mcp.agentic_mcp import auto_configure_agentic_mcp_server
            ...
            if "kluisz migration --fix" not in str(exc):
            ...
            from kluisz.__main__ import get_number_of_workers
            from kluisz.cli.progress import create_kluisz_shutdown_progress

            log_level = os.getenv("KLUISZ_LOG_LEVEL", "info").lower()
            num_workers = get_number_of_workers(get_settings_service().settings.workers)
            shutdown_progress = create_kluisz_shutdown_progress(
            ...
            try:
                ...
                with shutdown_progress.step(2):
                    try:
                        ...
                        # Catch recursion errors during teardown - may happen with complex PostgreSQL operations
                        await logger.awarning(f"RecursionError during teardown (continuing cleanup): {e}")
                    except Exception as e:
                        await logger.awarning(f"Error during teardown_services (continuing cleanup): {e}")
                ...
                with shutdown_progress.step(4):
                    await logger.adebug("Kluisz Kanvas shutdown complete")
...
def create_app():
    ...
    # Feature enforcement middleware - automatically protects routes based on ROUTE_FEATURE_MAP
    # See: docs/dev/tenant-features/EXTENSIBILITY_GUIDE.md - Pattern 2
    # To protect a new route, add it to route_features.py - no endpoint changes needed
    ...
    if prome_port_str := os.environ.get("KLUISZ_PROMETHEUS_PORT"):
    ...
    if settings.mcp_server_enabled:
        from kluisz.api.v1 import mcp_router
...
def get_static_files_dir():
    """Get the static files directory relative to Kluisz Kanvas's main.py file."""
