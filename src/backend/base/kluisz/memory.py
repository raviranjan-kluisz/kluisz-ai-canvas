...
from kluisz.services.database.models.message.model import MessageRead, MessageTable
from kluisz.services.deps import session_scope
...
async def aadd_messages(messages: Message | list[Message], flow_id: str | UUID | None = None):
    ...
    # Check if all messages are Message instances (either from kluisz or klx)
    # Use attribute checking instead of isinstance/class inspection to avoid SQLAlchemy inspection
    for message in messages:
        # Accept Message instances by checking for required attributes
        # This avoids SQLAlchemy inspection of ErrorMessage class
        is_valid_message = (
            hasattr(message, "text") and 
            hasattr(message, "sender") and 
            hasattr(message, "sender_name")
        )
