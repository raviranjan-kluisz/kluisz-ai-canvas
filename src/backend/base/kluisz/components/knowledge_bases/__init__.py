"""Langflow knowledge bases module - forwards to klx.components.files_and_knowledge.
...
from klx.components.files_and_knowledge import __all__ as _klx_all

__all__: list[str] = list(_klx_all)

# Register redirected submodules in sys.modules for direct importlib.import_module() calls
# This allows imports like: import kluisz.components.knowledge_bases.ingestion
_redirected_submodules = {
    "kluisz.components.knowledge_bases.ingestion": "klx.components.files_and_knowledge.ingestion",
    "kluisz.components.knowledge_bases.retrieval": "klx.components.files_and_knowledge.retrieval",
}
...
def __getattr__(attr_name: str) -> Any:
    """Forward attribute access to klx.components.files_and_knowledge."""
    # Handle submodule access for backwards compatibility
    if attr_name == "ingestion":
        from importlib import import_module

        result = import_module("klx.components.files_and_knowledge.ingestion")
        globals()[attr_name] = result
        return result
    if attr_name == "retrieval":
        from importlib import import_module

        result = import_module("klx.components.files_and_knowledge.retrieval")
        globals()[attr_name] = result
        return result

    from klx.components import files_and_knowledge

    return getattr(files_and_knowledge, attr_name)


def __dir__() -> list[str]:
    """Forward dir() to klx.components.files_and_knowledge."""
    return list(__all__)
