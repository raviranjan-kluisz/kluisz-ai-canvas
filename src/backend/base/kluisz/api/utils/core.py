...
from klx.services.deps import injectable_session_scope, injectable_session_scope_readonly, session_scope
from sqlalchemy import delete
from sqlmodel.ext.asyncio.session import AsyncSession

from kluisz.services.auth.utils import get_current_active_user, get_current_active_user_mcp
from kluisz.services.database.models.flow.model import Flow
from kluisz.services.database.models.message.model import MessageTable
...
from kluisz.services.store.utils import get_lf_version_from_pypi
from kluisz.utils.constants import KLUISZ_GLOBAL_VAR_HEADER_PREFIX

if TYPE_CHECKING:
    from kluisz.services.chat.service import ChatService
    from kluisz.services.store.schema import StoreComponentCreate
...
async def check_kluisz_version(component: StoreComponentCreate) -> None:
    ...
    kluisz_version = await get_lf_version_from_pypi()
    if kluisz_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of Kluisz Kanvas")
    if kluisz_version != component.last_tested_version:
        await logger.awarning(
            f"Your version of Kluisz Kanvas ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({kluisz_version}) and try again."
        )
...
async def build_graph_from_data(flow_id: uuid.UUID | str, payload: dict, **kwargs):
    ...
    # Set Kluisz-specific metadata for tracing if provided
    if "folder_id" in kwargs and kwargs["folder_id"]:
        graph.kluisz_folder_id = str(kwargs["folder_id"])
    if "tenant_id" in kwargs and kwargs["tenant_id"]:
        graph.kluisz_tenant_id = str(kwargs["tenant_id"])
...
async def build_graph_from_db_no_cache(flow_id: uuid.UUID, session: AsyncSession, **kwargs):
    ...
    graph = await build_graph_from_data(flow_id, flow.data, flow_name=flow.name, **kwargs)
    ...
    if flow.user_id:
        ...
        if user and user.tenant_id:
            graph.kluisz_tenant_id = str(user.tenant_id)
    return graph
...
async def verify_public_flow_and_get_user(flow_id: uuid.UUID, client_id: str | None) -> tuple[User, uuid.UUID]:
    ...
    async with session_scope() as session:
        ...
        from kluisz.services.database.models.flow.model import AccessTypeEnum, Flow
...
def extract_global_variables_from_headers(headers) -> dict[str, str]:
    ...
    try:
        for header_name, header_value in headers.items():
            ...
            if header_lower.startswith(KLUISZ_GLOBAL_VAR_HEADER_PREFIX):
                var_name = header_lower[len(KLUISZ_GLOBAL_VAR_HEADER_PREFIX) :].upper()
