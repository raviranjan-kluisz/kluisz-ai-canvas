...
from kluisz.api.disconnect import DisconnectHandlerStreamingResponse
from kluisz.api.utils import (
    CurrentActiveUser,
    EventDeliveryType,
...
from kluisz.api.v1.schemas import FlowDataRequest, ResultDataResponse, VertexBuildResponse
from kluisz.events.event_manager import EventManager
from kluisz.exceptions.component import ComponentBuildError
from kluisz.schema.message import ErrorMessage
from kluisz.schema.schema import OutputValue
from kluisz.services.database.models.flow.model import Flow
from kluisz.services.deps import get_chat_service, get_telemetry_service, session_scope
from kluisz.services.features import get_validation_service
from kluisz.services.job_queue.service import JobQueueNotFoundError, JobQueueService
from kluisz.services.telemetry.schema import ComponentInputsPayload, ComponentPayload, PlaygroundPayload
...
async def generate_flow_events(
    *,
    ...
    # Validate feature access for all components in the flow
    # This ensures users can only execute flows containing components they have access to
    try:
        validation_service = get_validation_service()
        flow_data_dict = {"nodes": [{"data": {"type": v.vertex_type}} for v in graph.vertices]}
        validation_result = await validation_service.validate_flow_execution(
            user_id=str(current_user.id),
        ...
        if not validation_result.allowed:
            raise HTTPException(
                status_code=403,
                detail=f"Flow contains components you don't have access to. Missing features: {', '.join(validation_result.missing_features)}",
            )
    except HTTPException:
        raise
    except Exception as e:
        # Log but don't block on validation errors - fail open for robustness
        await logger.awarning(f"Feature validation error (allowing execution): {e}")
