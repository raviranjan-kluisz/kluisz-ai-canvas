...
# Lazy import for elevenlabs - only loaded when voice features are used
_ElevenLabs = None


def _get_elevenlabs():
    """Lazy import of ElevenLabs to avoid import errors when elevenlabs is not installed."""
    global _ElevenLabs
    if _ElevenLabs is None:
        try:
            ...
            msg = (
                "elevenlabs is required for voice features. "
                "Install it with: pip install elevenlabs>=1.52.0"
            )
...
from kluisz.api.v1.chat import build_flow_and_stream
from kluisz.memory import aadd_messagetables
from kluisz.schema.properties import Properties
from kluisz.services.auth.utils import get_current_user_for_websocket
...
from kluisz.services.deps import get_variable_service, session_scope
from kluisz.utils.voice_utils import BYTES_PER_24K_FRAME, VAD_SAMPLE_RATE_16K, resample_24k_to_16k
                    ...
                    while True:
                        ...
                        if msg.get("type") == "input_audio_buffer.append":
                            ...
                            if num_audio_samples > AUDIO_SAMPLE_THRESHOLD:
                                ...
                                num_audio_samples = 0
                        elif msg.get("type") == "kluisz.voice_mode.config":
                            await logger.ainfo(f"kluisz.voice_mode.config {msg}")
                            voice_config.progress_enabled = msg.get("progress_enabled", True)
                        elif msg.get("type") == "kluisz.elevenlabs.config":
                            await logger.ainfo(f"kluisz.elevenlabs.config {msg}")
                ...
                try:
                    while True:
                        ...
                        if event.get("type") == "input_audio_buffer.append":
                            ...
                            openai_send(event)
                        elif event.get("type") == "kluisz.elevenlabs.config":
                            await logger.ainfo(f"kluisz.elevenlabs.config {event}")
