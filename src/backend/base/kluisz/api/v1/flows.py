...
from kluisz.api.utils import CurrentActiveUser, DbSession, cascade_delete_flow, remove_api_keys, validate_is_component
from kluisz.api.v1.schemas import FlowListCreate
from kluisz.services.limits import FlowLimitExceededError, get_limits_enforcement_service
...
from kluisz.services.database.models.flow.model import (
    AccessTypeEnum,
    Flow,
    FlowCreate,
    FlowHeader,
...
from kluisz.services.database.models.flow.utils import get_webhook_component_in_flow
...
async def create_flow(
    *,
    ...
    try:
        ...
        await limits_service.check_can_create_flow(str(current_user.id))
    except FlowLimitExceededError as exc:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={
                "message": exc.message,
                "error_type": "flow_limit_exceeded",
...
async def create_flows(
    *,
    ...
    # Check flow limit before creating (count how many flows we're creating)
    try:
        limits_service = get_limits_enforcement_service()
        limit_status = await limits_service.check_can_create_flow(str(current_user.id))
        if not limit_status.get("unlimited", False) and limit_status.get("remaining") is not None:
            flows_to_create = len(flow_list.flows)
            if flows_to_create > limit_status["remaining"]:
                raise FlowLimitExceededError(
                    user_id=str(current_user.id),
                    current_count=limit_status.get("current_count", 0),
                    max_allowed=limit_status.get("max_allowed", 0),
                    message=f"Cannot create {flows_to_create} flows. "
                            f"You have {limit_status['remaining']} flow slots remaining.",
                )
    except FlowLimitExceededError as exc:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def upload_file(
    *,
    ...
    # Check flow limit before importing
    try:
        limits_service = get_limits_enforcement_service()
        limit_status = await limits_service.check_can_create_flow(str(current_user.id))
        if not limit_status.get("unlimited", False) and limit_status.get("remaining") is not None:
            ...
            if flows_to_create > limit_status["remaining"]:
                raise FlowLimitExceededError(
                    ...
                    current_count=limit_status.get("current_count", 0),
                    max_allowed=limit_status.get("max_allowed", 0),
                    message=f"Cannot import {flows_to_create} flows. "
                            f"You have {limit_status['remaining']} flow slots remaining.",
                )
    except FlowLimitExceededError as exc:
        raise HTTPException(
