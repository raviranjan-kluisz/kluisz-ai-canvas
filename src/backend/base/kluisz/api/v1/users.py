...
from kluisz.api.v1.schemas import UsersResponse
from kluisz.initial_setup.setup import get_or_create_default_folder
from kluisz.services.auth.utils import (
    get_current_active_superuser,
    get_password_hash,
    verify_password,
)
from kluisz.services.database.models.user.crud import get_user_by_id, update_user
from kluisz.services.database.models.user.model import User, UserCreate, UserRead, UserUpdate
...
async def patch_user(
    ...
    if not user.is_platform_superadmin and user_update.is_platform_superadmin:
        raise HTTPException(status_code=403, detail="Permission denied")

    if not user.is_platform_superadmin and user.id != user_id:
        raise HTTPException(status_code=403, detail="Permission denied")
    if update_password:
