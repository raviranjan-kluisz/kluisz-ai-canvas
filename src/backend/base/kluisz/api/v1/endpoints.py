...
from klx.custom.utils import (
    add_code_field_to_build_config,
    build_custom_component_template,
    get_instance_name,
    update_component_build_config,
)
from klx.graph.graph.base import Graph
from klx.graph.schema import RunOutputs
from klx.log.logger import logger
from klx.schema.schema import InputValueRequest
from klx.services.settings.service import SettingsService
from sqlmodel import select

from kluisz.api.utils import CurrentActiveUser, DbSession, extract_global_variables_from_headers, parse_value
from kluisz.api.v1.schemas import (
    ConfigResponse,
...
from kluisz.events.event_manager import create_stream_tokens_event_manager
from kluisz.exceptions.api import APIException, InvalidChatInputError
from kluisz.exceptions.serialization import SerializationError
from kluisz.helpers.flow import get_flow_by_id_or_endpoint_name
from kluisz.interface.initialize.loading import update_params_with_load_from_db_fields
from kluisz.processing.process import process_tweaks, run_graph_internal
from kluisz.schema.graph import Tweaks
from kluisz.services.auth.utils import api_key_security, get_current_active_user, get_webhook_user
from kluisz.services.cache.utils import save_uploaded_file
from kluisz.services.credits.enforcement import (
    CreditEnforcementService,
    InsufficientCreditsError,
    NoActiveLicenseError,
    get_credit_enforcement_service,
)
from kluisz.services.limits.enforcement import (
    ApiCallLimitExceededError,
    get_limits_enforcement_service,
)
from kluisz.services.database.models.flow.model import Flow, FlowRead
from kluisz.services.database.models.flow.utils import get_all_webhook_components_in_flow
from kluisz.services.database.models.user.model import User, UserRead
from kluisz.services.deps import get_session_service, get_settings_service, get_telemetry_service
from kluisz.services.telemetry.schema import RunPayload
from kluisz.utils.compression import compress_response
from kluisz.utils.version import get_version_info

if TYPE_CHECKING:
    from kluisz.events.event_manager import EventManager
...
async def get_all():
    ...
    """
    from kluisz.interface.components import get_and_cache_all_types_dict
...
async def simple_run_flow(
    ...
    *,
    ...
    try:
        ...
        # Set Kluisz-specific metadata for tracing
        if flow.folder_id:
            graph.kluisz_folder_id = str(flow.folder_id)
        if api_key_user and api_key_user.tenant_id:
            graph.kluisz_tenant_id = str(api_key_user.tenant_id)
        if run_id is None:
            run_id = str(uuid4())
...
async def _run_flow_internal(
    *,
    ...
    Args:
        ...
        skip_credit_check (bool): Skip credit enforcement (for platform super admins)
    ...
    # Credit enforcement - check if user has sufficient credits
    # Skip for platform super admins or when explicitly disabled
    is_superadmin = getattr(api_key_user, "is_platform_superadmin", False)
    if not skip_credit_check and not is_superadmin:
        try:
            credit_service = get_credit_enforcement_service()
            await credit_service.check_user_can_execute(str(api_key_user.id))
        except NoActiveLicenseError as exc:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail={
                    "message": "You need an active license to execute flows",
                    "error_type": "no_active_license",
        ...
        # API call limit enforcement - check if user has remaining API calls for this period
        try:
            limits_service = get_limits_enforcement_service()
            await limits_service.check_api_call_limit(str(api_key_user.id))
        except ApiCallLimitExceededError as exc:
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail={
                    "message": exc.message,
                    "error_type": "api_call_limit_exceeded",
    ...
    run_id = str(uuid4())  # Generate run_id early for both streaming and non-streaming flows
    ...
    if stream:
        ...
        # Run flow - credit deduction is handled by the metering callback
        # automatically during the flow execution (see TracingService.get_langchain_callbacks)
    ...
    try:
        ...
        # Note: Credit deduction is now handled by the metering callback
        # automatically during flow execution (see TracingService.get_langchain_callbacks)
        # No need for manual deduction here!
...
# get endpoint to return version of kluisz kanvas
@router.get("/version")
