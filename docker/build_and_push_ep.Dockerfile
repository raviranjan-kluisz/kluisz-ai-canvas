...
RUN --mount=type=cache,target=/root/.npm \
    ...
    && cp -r build /app/src/backend/base/kluisz/frontend \
...
# curl is required for kluisz health checks
...
ENV KLUISZ_EVENT_DELIVERY=polling

USER 1000
CMD ["python", "-m", "kluisz", "run", "--host", "0.0.0.0", "--backend-only"]
