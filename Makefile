...
env ?= .env
open_browser ?= true
path = src/backend/base/kluisz/frontend
workers ?= 1
async ?= true
lf ?= false
ff ?= true
...
help: ## show basic help message with common commands
	@echo ''
	@echo "$(GREEN)═══════════════════════════════════════════════════════════════════$(NC)"
	@echo "$(GREEN)                    KLUISZ KANVAS MAKEFILE COMMANDS                     $(NC)"
	@echo "$(GREEN)═══════════════════════════════════════════════════════════════════$(NC)"
	@echo ''
...
clean_npm_cache:
	...
	$(call CLEAR_DIRS,src/frontend/node_modules src/frontend/build src/backend/base/kluisz/frontend)
...
clean_frontend_build: ## clean frontend build artifacts to ensure fresh build
	...
	$(call CLEAR_DIRS,src/backend/base/kluisz/frontend)
...
lint: install_backend ## run linters
	@uv run mypy --namespace-packages -p "kluisz"
...
setup_devcontainer: ## set up the development container
	...
	make install_frontend
	make build_frontend
	uv run kluisz --frontend-path src/frontend/build
...
ifdef login
	...
	KLUISZ_AUTO_LOGIN=$(login) uv run uvicorn \
		--factory kluisz.main:create_app \
		--host 0.0.0.0 \
		--port $(port) \
...
ifdef base
	...
	make build_frontend
	make build_kluisz_base args="$(args)"
endif
...
dockerfile_build_be: dockerfile_build
	...
	@$(DOCKER) build --rm \
		--build-arg KLUISZ_IMAGE=kluisz:${VERSION} \
		-f ${DOCKERFILE_BACKEND} \
		-t kluisz_backend:${VERSION} .
...
patch: ## Update version across all projects. Usage: make patch v=1.5.0
	...
	\
	KLUISZ_VERSION="$(v)"; \
	KLUISZ_BASE_VERSION=$$(echo "$$KLUISZ_VERSION" | sed -E 's/^[0-9]+\.(.*)$$/0.\1/'); \
	\
	echo "$(GREEN)Kluisz Kanvas version: $$KLUISZ_VERSION$(NC)"; \
	echo "$(GREEN)Kluisz-base version: $$KLUISZ_BASE_VERSION$(NC)"; \
	\
	echo "$(GREEN)Updating main pyproject.toml...$(NC)"; \
	python -c "import re; fname='pyproject.toml'; txt=open(fname).read(); txt=re.sub(r'^version = \".*\"', 'version = \"$$KLUISZ_VERSION\"', txt, flags=re.MULTILINE); txt=re.sub(r'\"kluisz-base==.*\"', '\"kluisz-base==$$KLUISZ_BASE_VERSION\"', txt); open(fname, 'w').write(txt)"; \
	\
	echo "$(GREEN)Updating kluisz-base pyproject.toml...$(NC)"; \
	python -c "import re; fname='src/backend/base/pyproject.toml'; txt=open(fname).read(); txt=re.sub(r'^version = \".*\"', 'version = \"$$KLUISZ_BASE_VERSION\"', txt, flags=re.MULTILINE); open(fname, 'w').write(txt)"; \
	\
	echo "$(GREEN)Updating frontend package.json...$(NC)"; \
	python -c "import re; fname='src/frontend/package.json'; txt=open(fname).read(); txt=re.sub(r'\"version\": \".*\"', '\"version\": \"$$KLUISZ_VERSION\"', txt); open(fname, 'w').write(txt)"; \
	\
	echo "$(GREEN)Validating version changes...$(NC)"; \
	if ! grep -q "^version = \"$$KLUISZ_VERSION\"" pyproject.toml; then echo "$(RED)✗ Main pyproject.toml version validation failed$(NC)"; exit 1; fi; \
	if ! grep -q "\"kluisz-base==$$KLUISZ_BASE_VERSION\"" pyproject.toml; then echo "$(RED)✗ Main pyproject.toml kluisz-base dependency validation failed$(NC)"; exit 1; fi; \
	if ! grep -q "^version = \"$$KLUISZ_BASE_VERSION\"" src/backend/base/pyproject.toml; then echo "$(RED)✗ Kluisz-base pyproject.toml version validation failed$(NC)"; exit 1; fi; \
	if ! grep -q "\"version\": \"$$KLUISZ_VERSION\"" src/frontend/package.json; then echo "$(RED)✗ Frontend package.json version validation failed$(NC)"; exit 1; fi; \
	...
	echo "  - pyproject.toml: $$KLUISZ_VERSION"; \
	echo "  - src/backend/base/pyproject.toml: $$KLUISZ_BASE_VERSION"; \
	echo "  - src/frontend/package.json: $$KLUISZ_VERSION"; \
...
locust: ## run locust load tests (options: locust_users=10 locust_spawn_rate=1 locust_host=http://localhost:7860 locust_headless=true locust_time=300s locust_api_key=your-api-key locust_flow_id=your-flow-id locust_file=src/backend/tests/locust/locustfile.py locust_min_wait=2000 locust_max_wait=5000 locust_request_timeout=30.0)
	...
	export KLUISZ_HOST=$(locust_host) && \
...
load_test_setup: ## Set up load test environment with starter project flows
	@echo "$(YELLOW)Setting up Kluisz Kanvas load test environment$(NC)"
...
load_test_kluisz_quick: ## Quick Kluisz Kanvas load test (10 users, 30s) with HTML report (automatically sets up if needed). Use FLOW_NAME="Flow Name" to specify flow
	@echo "$(YELLOW)Running quick Kluisz Kanvas load test with HTML report$(NC)"
	@if [ ! -f "src/backend/tests/locust/load_test_creds.json" ]; then \
		echo "$(BLUE)No credentials found. Running automatic setup...$(NC)"; \
		if [ -z "$(FLOW_NAME)" ]; then \
			echo "$(CYAN)Available flows:$(NC)"; \
			cd src/backend/tests/locust && uv run python langflow_setup_test.py --list-flows; \
			echo "$(RED)Please specify a flow: make load_test_kluisz_quick FLOW_NAME=\"Basic Prompting\"$(NC)"; \
			exit 1; \
		else \
			echo "$(BLUE)Setting up with flow: $(FLOW_NAME)$(NC)"; \
			cd src/backend/tests/locust && uv run python langflow_setup_test.py --flow "$(FLOW_NAME)" --save-credentials load_test_creds.json; \
...
load_test_help: ## Show detailed load testing help
	...
	@echo "  load_test_kluisz_quick - Quick Kluisz Kanvas test (10 users, 30s)"
...
help_backend: ## show backend-specific commands
	...
	@echo "  $(GREEN)make build_kluisz_base$(NC) - Build kluisz-base package"
	@echo "  $(GREEN)make build_kluisz$(NC)      - Build kluisz package"
...
help_advanced: ## show advanced and miscellaneous commands
	...
	@echo "    This updates: pyproject.toml, kluisz-base, frontend package.json"
	@echo ''
	...
	@echo "  $(GREEN)make publish_base$(NC)        - Publish kluisz-base to PyPI"
	@echo "  $(GREEN)make publish_kluisz$(NC)    - Publish kluisz to PyPI"
	...
	@echo "  $(GREEN)make lock_base$(NC)           - Lock kluisz-base dependencies"
	@echo "  $(GREEN)make lock_kluisz$(NC)       - Lock kluisz dependencies"
	@echo ''
