...
if [ ! -f "src/klx/pyproject.toml" ]; then
    print_error "This script must be run from the root of the langflow repository"
    exit 1
fi

# Get current version
CURRENT_VERSION=$(grep '^version = ' src/klx/pyproject.toml | cut -d'"' -f2)
print_info "Current KLX version: $CURRENT_VERSION"
...
print_info "Preparing to release KLX version $NEW_VERSION"
...
if [ "$DRY_RUN" = true ]; then
    ...
    sed -i.bak "s/^version = \".*\"/version = \"$NEW_VERSION\"/" src/klx/pyproject.toml
    rm src/klx/pyproject.toml.bak
fi

# Update version in Dockerfiles if they have ARG KLX_VERSION
if grep -q "ARG KLX_VERSION" src/klx/docker/Dockerfile 2>/dev/null; then
    if [ "$DRY_RUN" = true ]; then
        print_dry_run "Would update version in Dockerfiles to $NEW_VERSION"
    else
        print_info "Updating version in Dockerfiles..."
        sed -i.bak "s/ARG KLX_VERSION=.*/ARG KLX_VERSION=$NEW_VERSION/" src/klx/docker/Dockerfile*
        rm src/klx/docker/Dockerfile*.bak
    fi
fi
...
if [ "$DRY_RUN" = true ]; then
    ...
    git add src/klx/pyproject.toml src/klx/docker/Dockerfile* 2>/dev/null || true
